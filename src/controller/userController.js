...
const secretkey = "TodoApp-456#Joy_bhattacharya@123"
...
const login = async function (req, res) {
    try {
        ...
        const resSend = await Auth(email, "Joy'S foundation.")
...
const enterOtp = async function (req, res) {
    try {
        ...
        if (!otpFromHeader) {
            return res.status(400).send({ status: false, message: "OTP not found in request header." })
        }

        if (!userIdFromHeader) {
            return res.status(400).send({ status: false, message: "userId not found in request header." })
        }
        ...
        if (otpFromHeader != otpFromBody) {
            return res.status(400).send({ status: false, message: `Login Failed! Incorrect OTP.` })
        }
        ...
        return res.status(200).send({ status: true, data: { userId: userIdFromHeader, token: token } })
